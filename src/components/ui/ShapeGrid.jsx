...
const ShapeGrid = ({
  ...
  useEffect(() => {
    ...
    const drawGrid = () => {
      ...
      if (isHex) {
        ...
        const cols = Math.ceil(canvas.offsetWidth / hexHoriz) + 3;
        const rows = Math.ceil(canvas.offsetHeight / hexVert) + 3;
        ...
        const cols = Math.ceil(canvas.offsetWidth / halfW) + 4;
        const rows = Math.ceil(canvas.offsetHeight / squareSize) + 4;
        ...
        const cols = Math.ceil(canvas.offsetWidth / squareSize) + 3;
        const rows = Math.ceil(canvas.offsetHeight / squareSize) + 3;
    ...
    const updateAnimation = () => {
      const effectiveSpeed = Math.max(speed, 0); // Allow 0 for no movement
    ...
    const handleMouseMove = event => {
      // If hoverFillColor is transparent, no need to track hover
      if (hoverFillColor === 'transparent') return;
  ...
  }, [direction, speed, borderColor, hoverFillColor, squareSize, shape, hoverTrailAmount, lineWidth]);
